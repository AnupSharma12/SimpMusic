 ...
 * - auvro://open-app?url=<encoded_url>  (redirected from website)
 * - auvro://watch?v=VIDEO_ID            (direct scheme)
 * - auvro://playlist?list=PLAYLIST_ID   (direct scheme)
 * - auvro://channel/CHANNEL_ID          (direct scheme)
 * - auvro://album?id=ALBUM_ID           (direct scheme)
 * - https://auvro.org/app/...            (web URL passed via args)
 */
object DesktopDeepLinkHandler {
     ...
     *
     * Conversion rules:
     * 1. auvro://open-app?url=<encoded_url>
     *    → Extract the `url` param and use it as intent data
     *
     * 2. auvro://watch?v=xxx, auvro://playlist?list=xxx, etc.
     *    → Convert to https://auvro.org/app/watch?v=xxx format
     *      so App.kt handles it uniformly via the auvro.org branch
     *
     * 3. https://auvro.org/app/... or YouTube URLs
     *    → Pass through as-is
     */
    private fun parseToIntent(uri: String): GenericIntent {
        ...
        val actualUri = when {
            // auvro://open-app?url=<encoded_url>
            parsed.scheme == "auvro" && parsed.host == "open-app" -> {
                val urlParam = parsed.getQueryParameter("url")
                if (urlParam != null) {
                    Logger.d(TAG, "Extracted URL from open-app: $urlParam")
                    Uri.parse(urlParam)
                } else {
                    // auvro://open-app without params → just open the app, no navigation
                    Logger.d(TAG, "open-app without URL param, just opening app")
                    null
                }
            }

            // auvro://watch?v=xxx → https://auvro.org/app/watch?v=xxx
            // auvro://playlist?list=xxx → https://auvro.org/app/playlist?list=xxx
            // auvro://channel/UCxxx → https://auvro.org/app/channel/UCxxx
            // auvro://album?id=xxx → https://auvro.org/app/album?id=xxx
            parsed.scheme == "auvro" && parsed.host != null -> {
                val host = parsed.host!!
                val query = parsed.query?.let { "?$it" } ?: ""
                val pathSuffix = parsed.pathSegments.joinToString("/").let {
                    if (it.isNotEmpty()) "/$it" else ""
                }
                val convertedUrl = "https://auvro.org/app/$host$pathSuffix$query"
                Logger.d(TAG, "Converted auvro:// to: $convertedUrl")
                Uri.parse(convertedUrl)
            }

            // https://auvro.org/app/... or YouTube URLs → pass through
            else -> parsed
        }
