...
/**
 * Registers the "auvro://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\auvro
 *     (Default) = "URL:Auvro Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "auvro"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering auvro:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:Auvro Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\Auvro.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/Auvro.exe
        // So we go: java.home → parent (runtime) → parent (app) → Auvro.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "Auvro.exe")
            if (exeFile.exists()) {
